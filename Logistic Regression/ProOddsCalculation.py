...
def show1():
   ...
   for index, row in enumerate(cursor2):
       ...
       odd = math.exp(y)
       proodd = odd/(odd+1)
       listBox1.insert("", "end", values=(row[0],row[1],y,math.exp(y),proodd))
        
   
scores = tk.Tk()
scores['bg']='#436fba'
label1 = tk.Label(scores, text="Pro Odds Catculation", font=("Arial bold",10)).grid(row=0, columnspan=1)
cols = ('Hours(x)', 'Pass(y)', 'Logit','Odds','Pro')
