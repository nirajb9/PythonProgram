...
def show1():
   ...
   for index, row in enumerate(cursor2):
       y= (m*row[0])+b
       listBox1.insert("", "end", values=(row[0],row[1],y))
        
   
scores = tk.Tk()
scores['bg']='#436fba'
label1 = tk.Label(scores, text="Logit Catculation", font=("Arial bold",10)).grid(row=0, columnspan=1)
cols = ('Hours(x)', 'Pass(y)', 'Logit')
