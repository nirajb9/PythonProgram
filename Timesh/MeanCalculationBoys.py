...
def show1():
   ...
   CalculatetTest(GMean,BMean,GSD,BSD,len(lst2))
   x=1
   for i in lst2:
       listBox1.insert("", "end", values=(x, i,i-BMean, (i-BMean)*(i-BMean)))
       x=x+1
   listBox2.insert("", "end", values=(1, "The population mean",BMean))
   listBox2.insert("", "end", values=(2, "population standard deviation",GSD))
   listBox2.insert("", "end", values=(3, "the size of the population",len(lst2)))
      
scores = tk.Tk()
scores['bg']='#e303fc'
label1 = tk.Label(scores, text="Calculating Mean Score for Boys", font=("Arial bold",10)).grid(row=0, columnspan=1)
cols = ('SNo', 'Boys(Yi)', '(Yi-Ymean)', 'Sqr(Yi-Ymean)')
